...
public class SensorCommand extends AbstractCommand {
  ...
  public <K extends SensorData> K getResponse(byte[] response) {
    try {
      Constructor<? extends SensorData> responseConstructor =
          sensorPacket.responseClassType.getConstructor(String.class, byte[].class);
      return (K) responseConstructor.newInstance(sensorPacket.packetName, response);
    } catch (Exception e) {
      throw new IRobotCommandException("This shouldn't have happened but if it did, contact me at emantos@gmail.com");
    }
  }
}
