...
public class GroupPacket3 extends AbstractSensorData {
  ...
  public GroupPacket3(String packetName, byte[] response) {
    ...
    batteryCapacity =
        new BatteryCapacity(SensorPacket.BATTERY_CAPACITY.packetName, new byte[] {response[8], response[9]});
  }
  ...
  public String toString() {
    return String.format("[%s] = [%s, %s, %s, %s, %s]", getPacketName(), chargingState.toString(), voltage.toString(),
        current.toString(), batteryTemperature.toString(), batteryCharge.toString(), batteryCapacity.toString());
  }
}
